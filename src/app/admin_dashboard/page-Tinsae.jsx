...
export default function AdminDashboardPage() {
  ...
  function refetchAll() {
    ...
    fetch("/api/admin/phishing-clicks")
      .then((res) => res.json())
      .then((data) => data.success && setClicks(data.clicks));
  }
  ...
  if (!account || !templates || !results || !clicks) {
    return <main className="min-h-screen bg-[#f0f4fb]" />;
  }

  const totalPhishing = templates.filter((t) => t.isPhishing).length;
  const compromisedCount = results.filter((r) => r.phishingFallenFor > 0).length;
  const totalClicks = clicks.filter((c) => c.clicked).length;
  ...
  return (
    <main className="min-h-screen bg-[#f0f4fb] text-[#1a1c1f]">
      ...
      <section className="hero-banner px-4 sm:px-6 py-10">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <HeroStat value={totalClicks} label="Phishing Link Clicks" yellow />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase text-gray-400 border-b border-gray-100">
                  ...
                  <th className="py-2 pr-3">Phishing</th>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase text-gray-400 border-b border-gray-100">
                  ...
                  <th className="py-2 pr-3">Phishing Risk</th>
                </tr>
              </thead>
              <tbody>
                {[...results]
                  .sort((a, b) => b.phishingFallenFor - a.phishingFallenFor)
                  .map((r) => (
                    <tr key={r.employeeId} className="border-b border-gray-50">
                      ...
                      <td className="py-2.5 pr-3">
                        <RiskBadge fallenFor={r.phishingFallenFor} total={r.phishingTotal} />
                      </td>
                    </tr>
                  ))}
              </tbody>
        ...
        <div className="section-card p-6">
          <h2 className="hg text-lg font-bold text-gray-900 mb-1">Phishing Link Clicks</h2>
          <p className="text-gray-500 text-xs mb-5">
            Per-employee tracking of every click on a simulated phishing link, across all three
            phishing emails.
          </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase text-gray-400 border-b border-gray-100">
                  <th className="py-2 pr-3">Employee</th>
                  <th className="py-2 pr-3">Phishing Email</th>
                  <th className="py-2 pr-3">Read</th>
                  <th className="py-2 pr-3">Clicked</th>
                  <th className="py-2 pr-3">Clicked At</th>
                </tr>
              </thead>
              <tbody>
                {[...clicks]
                  ...
                  .map((c) => (
                    <tr key={`${c.employeeId}-${c.emailId}`} className="border-b border-gray-50">
                      <td className="py-2.5 pr-3 font-semibold text-gray-800 whitespace-nowrap">{c.fullName}</td>
                      <td className="py-2.5 pr-3 text-gray-600 max-w-[280px] truncate">{c.subject}</td>
                      <td className="py-2.5 pr-3 text-gray-500">{c.isRead ? "Yes" : "No"}</td>
                      <td className="py-2.5 pr-3">
                        {c.clicked ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-50 text-rose-600">
                            <span className="material-symbols-outlined text-sm">touch_app</span>
                            Clicked
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-700">
                            Not clicked
                          </span>
                        )}
                      </td>
                      <td className="py-2.5 pr-3 text-gray-500 whitespace-nowrap">
                        {c.clickedAt ? formatDate(c.clickedAt) : "—"}
                      </td>
                    </tr>
                  ))}
              </tbody>
            ...
            {clicks.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-8">
                No phishing emails have been delivered yet.
              </p>
            )}
          </div>
        </div>
...
function RiskBadge({ fallenFor, total }) {
  if (total === 0) {
    return <span className="text-gray-400 text-xs">No phishing emails sent yet</span>;
  }
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold ${palette}`}>
      {fallenFor} / {total} fell for it
    </span>
  );
}
